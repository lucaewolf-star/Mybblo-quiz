...
**Informazione tecnica importante per il developer — pagina di conferma azioni:**

Tutti e tre i bottoni di azione (Check-up confermato, Rifiuto o rinvio, Blocca Email B) usano lo **stesso file HTML**: `mybblo_conferma_azione.html`. La pagina cambia contenuto in base al parametro `azione` passato nell'URL. I link da inserire nel briefing e nella notifica giorno 10 devono avere questa struttura:

- **Check-up confermato:** `mybblo_conferma_azione.html?azione=confermato&nome={{NOME_CLIENTE}}&token={{TOKEN}}`
- **Rifiuto o rinvio:** `mybblo_conferma_azione.html?azione=rifiuto&nome={{NOME_CLIENTE}}&token={{TOKEN}}`
- **Blocca Email B:** `mybblo_conferma_azione.html?azione=blocca&nome={{NOME_CLIENTE}}&token={{TOKEN}}`

Il file `mybblo_conferma_azione.html` deve essere hostato su GitHub Pages come gli altri file HTML. Il parametro `token` è il token univoco per contatto generato dalla funzione serverless — senza token valido la funzione non deve eseguire nulla. La pagina di conferma e la schermata di esito ("Fatto") sono già sviluppate visivamente per tutte e tre le varianti — il developer deve solo sostituire il placeholder interno con la chiamata reale alla funzione serverless.
